    ...
    >
      <div class="container mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
          <a
            (click)="scrollToSection.scrollToSection('home', $event)"
          ...
          >
            ...
            <lucide-icon *ngIf="isMobileMenuOpen()" [name]="X" class="h-6 w-6">
            </lucide-icon>
          </button>
        </div>
      </div>
          ...
          <nav class="flex flex-col space-y-4">
            <a
              *ngFor="let item of navItems"
              [href]="item.href"
              (click)="
                scrollToSection.scrollToSection(item.href.substring(1), $event);
                closeMobileMenu()
              "
